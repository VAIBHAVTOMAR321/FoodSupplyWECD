...
const DirectorHCMReceiving = () => {
  ...
  const fetchBeneficiarySummary = useCallback(async () => {
    ...
    setError("");
    try {
      const response = await api.get(`/director/hcm-receiving-summary/`);
  ...
  const exportToExcel = () => {
    ...
    const quantityTotalsData = Object.entries(totals.quantityByUnit).map(([unit, total]) => ({
      'Unit': unit, 'Total Quantity': total.toFixed(2)
    }));

    const worksheet = XLSX.utils.json_to_sheet(dataToExport);
    const quantityWorksheet = XLSX.utils.json_to_sheet(quantityTotalsData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "HCM Receiving Report");
            ...
            {loading ? (
              ...
              >
                <Tab eventKey="district_summary" title="District Summary">
                  <h5 className="mt-4">District-wise Summary (Total)</h5>
                  <Table striped bordered hover responsive>
                    ...
                    <tfoot>
                      <tr className="fw-bold table-info">
                        ...
                        <td colSpan="2">
                          <Button variant="link" size="sm" onClick={() => handleViewTotals(summaryData.district_summary, 'District Summary Totals', 'total_quantity')}>View Totals</Button>
                        </td>
                      </tr>
                    </tfoot>
                  </Table>
                ...
                <Tab eventKey="project_summary" title="Project Summary">
                  ...
                  <Table striped bordered hover responsive className="mb-4">
                    ...
                    <tbody>
                      {summaryData.project_summary.map((project, index) => (
                    ...
                    <tfoot>
                      <tr className="fw-bold table-info">
                        ...
                        <td colSpan="2">
                          <Button variant="link" size="sm" onClick={() => handleViewTotals(summaryData.project_summary, 'Project Summary Totals', 'total_quantity')}>View Totals</Button>
                        </td>
                      </tr>
                    </tfoot>
                  </Table>
                  ...
                  <Table striped bordered hover responsive className="mb-4">
                    ...
                    <tfoot>
                      <tr className="fw-bold table-info">
                        <td colSpan="3">Total</td>
                        <td colSpan="2">
                          <Button variant="link" size="sm" onClick={() => handleViewTotals(summaryData.sector_summary, 'Sector Summary Totals', 'total_quantity')}>View Totals</Button>
                        </td>
                      </tr>
                    </tfoot>
                  </Table>
                ...
                <Tab eventKey="awc_report" title="AWC Report">
                  ...
                  {filteredReports.length === 0 ? (
                    <div className="text-center py-4 text-muted">
                      No HCM receiving reports found for the selected filters.
                    </div>
                  ) : (
              ...
              <Table striped bordered>
                ...
                <tbody>
                  {Object.entries(modalData.totals).map(([unit, total]) => (
                    <tr key={unit}><td>{unit}</td><td>{parseFloat(total).toFixed(2)}</td></tr>
                  ))}
                </tbody>
              </Table>
...
export default DirectorHCMReceiving;